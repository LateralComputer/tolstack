...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("Monte Carlo 100,000 samples", |b| {
        b.iter(|| mc(black_box(100_000)))
    });
    c.bench_function("Monte Carlo 1,000,000 samples", |b| {
        b.iter(|| mc(black_box(1_000_000)))
    });
    c.bench_function("Monte Carlo 10,000,000 samples", |b| {
        b.iter(|| mc(black_box(10_000_000)))
    });
}
...
fn mc(samples: usize) {
    ...
    analysis::monte_carlo(&stack, samples);
}
